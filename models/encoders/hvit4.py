# Process unshifted & shifted windows in parallel.
...
class StackShiftedWindow(nn.Module):
    ...
    def __init__(self, in_size, ctx_size):
        ...
        attn_mask = torch.cat([torch.zeros_like(attn_mask), attn_mask]) # [g n n] -> [(2 g) n n]
    ...
    def stack(self, x, ps):
        ...
        xs = torch.roll(x, shifts=self.shifts, dims=self.dims)

        x = self.split(x) # [b N1 ... Nd d] -> [b g1 ... gd n d]
        xs = self.split(xs) # [b N1 ... Nd d] -> [b g1 ... gd n d]
        x, self.ps1 = pack([x, xs], 'b * n d') # 2 x [b g1 ... gd n d] -> [b g n d]
        x, self.ps2 = pack([x], '* n d') # [b g n d] -> [B n d]
        return x
    
    def unstack(self, x):
        """x: [B n d] -> [B n d]"""
        [x] = unpack(x, self.ps2, '* n d') # [B n d] -> [b g n d]
        [x, xs] = unpack(x, self.ps1, 'b * n d') # [b g n d] -> 2 x [b g1 ... gd n d]
        x = self.merge(x) # [b g1 ... gd n d] -> [b N1 ... Nd d]
        xs = self.merge(xs) # [b g1 ... gd n d] -> [b N1 ... Nd d]

        xs = torch.roll(xs, shifts=self.unshifts, dims=self.dims)
        x = (x + xs) / 2
...
class Stage(nn.Module):
    ...
    def __init__(
        ...
        self.stack_shift = StackShiftedWindow(in_size, ctx_size) if use_shifted_window else None
...
def get_args_parser():
    parser = ArgumentParser("hvit4", add_help=False)
    ...
    parser.add_argument("--use_shifted_window") # list[bool]
