...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.out_dim = embed_dim * 2 ** (self.num_layers - 1)
...
def get_args_parser():
    parser = ArgumentParser("hvit", add_help=False)
    parser.add_argument("--patch_size", type=int)
    parser.add_argument("--embed_dim", type=int)
    parser.add_argument("--depths") # list[int]
    parser.add_argument("--num_heads") # list[int]
...
def create_encoder(config_root, config, in_channels, in_size, n_classes):
    ...
    assert all(s == in_size[0] for s in in_size)
