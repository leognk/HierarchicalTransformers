...
from .sft.size_dynamics import PipelineDynamics
import utils
...
class QueryInitializer(nn.Module):
    ...
    def __init__(self, method, ctx_size, qry_size, in_dim, out_dim):
        ...
        elif method == 'ctx_proj': self.initializer = CtxProjQueryInitializer(ctx_size, qry_size, in_dim, out_dim)
        elif method == 'linear_pool': self.initializer = LinearPoolQueryInitializer(ctx_size, qry_size, in_dim, out_dim)
...
class OneStepCodec(nn.Module):
    ...
    def __init__(self, n_layers, attend_to_query, in_dim, out_dim, mlp_hidden_ratio, n_heads, dropout, use_flash_attn):
        super().__init__()
        if attend_to_query is None:
            attend_to_query = True
        self.attend_to_query = attend_to_query
        self.in_dim = in_dim
        self.out_dim = out_dim
        mlp_hidden_dim = int(in_dim * mlp_hidden_ratio)
        self.transformer = Transformer(n_layers, attend_to_query, in_dim, mlp_hidden_dim, n_heads, dropout, None, use_flash_attn)
        if in_dim != out_dim:
            self.proj = nn.Sequential(
                nn.LayerNorm(in_dim),
                nn.Linear(in_dim, out_dim),
...
class TwoStepCodec(nn.Module):
    ...
    def __init__(self, n_layers_transformer, n_layers_codec, attend_to_query, in_dim, out_dim, mlp_hidden_ratio, n_heads, dropout, use_flash_attn):
        super().__init__()
        self.attend_to_query = attend_to_query
        self.in_dim = in_dim
        self.out_dim = out_dim
        mlp_hidden_dim = int(in_dim * mlp_hidden_ratio)
        self.transformer = Transformer(n_layers_transformer, True, in_dim, mlp_hidden_dim, n_heads, dropout, None, use_flash_attn)
        self.codec = Transformer(n_layers_codec, False, out_dim, mlp_hidden_dim, n_heads, dropout, None, use_flash_attn)
        if in_dim != out_dim:
            self.proj = nn.Sequential(
                nn.LayerNorm(in_dim),
                nn.Linear(in_dim, out_dim),
...
class Codec(nn.Module):
    ...
    def __init__(
        ...
        if self.use_qry: assert len(ctx_size) == len(qry_size)
        else: assert len(n_layers) == 1
        ...
        if self.use_qry: self.encoding = utils.product(qry_size) <= utils.product(ctx_size)
        if self.use_qry:
            self.scale_factor = utils.div_it(qry_size, ctx_size)
        else:
            n_axes = len(ctx_size if ctx_size is not None else qry_size)
            self.scale_factor = [1] * n_axes

        self.splitter = GroupSplitter(ctx_size if ctx_size else qry_size)
        self.merger = GroupMerger(qry_size if qry_size else ctx_size)
        if self.use_qry and forward:
        ...
        self.kstep_codec = KStepCodec(*n_layers, attend_to_query, in_dim, out_dim, mlp_hidden_ratio, n_heads, dropout, use_flash_attn)
    ...
    def forward(self, x, context, real_size, stash):
        ...
        if self.use_qry and x is None: x = self.qry_initializer(context)
        if self.use_qry:
            # The query becomes the next context and the context becomes the next query.
            context, x = self.kstep_codec(x, context)
        else:
            _, context = self.kstep_codec(x=None, context=context)
...
def get_args_parser():
    parser = ArgumentParser("attn_pool2", add_help=False)
